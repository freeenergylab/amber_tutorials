...
# using pymol to shift ligand FRH away from binding site => FRH_A_pymol_shift.pdb
```
...
mkdir -p ligand
cd ligand
$AMBERHOME/bin/antechamber -i FRH_A_pymol_shift.pdb -fi pdb -o SP4206_shift.mol2 -fo mol2 -at gaff2 -c bcc -an n -nc 0 -rn MOL -dr no -pf y
$AMBERHOME/bin/parmchk2 -i SP4206_shift.mol2 -f mol2 -o SP4206.frcmod -a Y -p $AMBERHOME/dat/leap/parm/gaff2.dat
grep "ATTN, need revision" SP4206.frcmod

mkdir -p to_sdf_rdkit
cd to_sdf_rdkit
$AMBERHOME/bin/antechamber -i ../FRH_A_pymol_shift.pdb -fi pdb -o SP4206_sybyl_shift.mol2 -fo mol2 -at sybyl -c bcc -an n -nc 0 -rn MOL -dr no -pf y
